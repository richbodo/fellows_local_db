...
**Deploy / PWA drift:** `just drift` prints prod's git SHA (from `/build-meta.json`) alongside local `HEAD` and `origin/main`, all SHA-aligned. After deploy, use the in-app **Diagnostics** panel (`?diag=1` or the Diagnostics button) to compare `/api/auth/status`, `/api/debug/diagnostics`, and `/build-meta.json` with the `X-Fellows-Build` response header (which carries the build timestamp — useful for correlating a single browser request with the matching server log line in journald). On the app server, `just prod-logs` (which runs `journalctl -u fellows-pwa -f`) shows `event=auth_status` JSON for each auth status request and `event=build_meta` at process start.
