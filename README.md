...
See `[docs/Architecture.md](docs/Architecture.md)` for fellows_local_db's specialization-and-conformance layer (axis picks, schema, HTTP routes, debug placeholders). The universal PNA architecture it cross-links into lives in the [`personal_network_toolkit`](https://github.com/richbodo/personal_network_toolkit) repo — `PNA_Spec.md`, `axes.md`, `use_cases.md`, and the typed contracts under `spec/contracts/`.
