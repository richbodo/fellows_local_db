...
- **Retire dev-only API routes — and migrate the e2e fixtures that depend on them.** `app/server.py`'s `/api/groups` and `/api/settings` handlers (POST/PATCH/DELETE/PUT and the matching GETs) are deleted. They become dead code with the worker as sole owner; keeping them creates a dev/prod asymmetry that drifts. The dev server still serves `/api/fellows*`, `/api/search`, `/api/stats`, static, images, and the auth/diagnostics stubs. **Test migration is part of P1**, not a follow-up: `tests/test_api.py` (delete the `groups`/`settings` test classes) and 7 e2e tests that use these routes as fixture setup (`test_copy_buttons.py`, `test_groups_compose.py`, `test_groups_index.py`, `test_groups_edit.py`, `test_groups_export.py`, `test_groups_detail.py`, `test_settings.py`) — rewrite their setup to drive the worker via `page.evaluate('window.__dataProvider.createGroup(...)')` (or whichever shim P1 lands). Add a single helper in `tests/e2e/conftest.py` so the setup pattern doesn't sprawl across files.
...
- A browser without SAH (e.g., a stale Safari) lands on `renderLocalDataUnavailablePanel` with the right copy. No silent degrade.
...
- Existing `tests/e2e/test_email_gate.py` deploy-server-mode harness verified working (cold-start tests need it).
- A clean Safari 16.4+ device or VM available for the manual SAH-fail acceptance check.
